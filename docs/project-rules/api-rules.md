...
Keep the layer (`middleware`, `routes`, `utils`, `constants`, `auth`, `openapi`, `test`). When a domain has **more than one file** in that layer (implementation, helpers, tests), nest them in a subdomain folder. Colocate `*.test.ts` next to the code they cover.
...
Runner: Node `node:test` + `node:assert/strict` (`pnpm --filter @relay/api test`). Do not add Vitest, Jest, or Supertest.

- Shared HTTP harness: `src/test/http.ts` — `listen`, `close`, `cookieHeader`, `register`, `canRun`, `Envelope`
- HTTP tests import from `@/test/http.js`. Pass a custom app when needed: `listen(createApp({ docs: false }))` or `listen(app)`
- Do not copy `listen` / `close` / cookie / register helpers into new tests
...
const user = await register(origin, { name, email, password });

// ❌ BAD — local copies of the harness
...
- **Source of truth:** Zod on the route (body/query), `sendSuccess` / `sendError` (envelope), `ErrorCode` in `constants/http.constant.ts` (failure identity)
