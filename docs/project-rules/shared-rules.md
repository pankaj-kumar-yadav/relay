...
**Exceptions (only):** same-directory (`./tokens.js`), package names (`express`, `zod`, `@prisma/client`, `@relay/shared`), one-level sibling only when tightly colocated — prefer `@/` when unsure.
...
|-----|--------|-----------|
| Shared | `packages/shared/src/constants/` | `<domain>.constant.ts` |
...
- Circle leftovers (reviews, initiatives, views) share `workspace.constant.ts` until they are first-class domains
...
Values used on both web and API live in `packages/shared/src/constants/` (`@relay/shared/constants/<domain>.constant`). Group shared source by kind (`constants/`, later `types/`, `components/`) — do not drop files in `src/` root. Do not add a kitchen-sink barrel. New shared files use `*.constant.ts`.

App `constants/` files exist **only** for app-only extras (path builders, cookie names, display copy, `JSON_BODY_LIMIT`). Do not add a re-export-only shim. Mixed domain files may re-export the shared names so one import can pull extras + shared values.

```ts
// ✅ shared value
import { OrgRole } from '@relay/shared/constants/org.constant';

// ✅ app extra (web team file also re-exports DEFAULT_TEAM_KEY)
import { teamHomePath } from '@/constants/team.constant';
```
...
- Branch on `NodeEnv` from `@relay/shared/constants/env.constant`
